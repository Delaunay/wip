"""
    ...
    The game engine will only manipulate numbers and no reference to the actual name should be made.
    You can use `Powers` and `Provinces` to access the different struct

    To make the game engine able to process custom diplomacy boards one needs to implement
    the current module.
"""
...
PROVINCE_DB = make_adjacency_list()


def province_from_string(name: str) -> Provinces:
    return PROVINCE_DB[Provinces[name]]
...
if __name__ == '__main__':
    ...
    print(PROVINCE_DB)
    print(LAND_TILES)
    print(SUPPLY_CENTERS)

    print(province_from_string('GAS'))
